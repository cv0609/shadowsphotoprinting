...
class PaymentController extends Controller
{
    ...
    public function createCustomer(Request $request)
    {
        ...
        // Check if user has referral code and validate email uniqueness
        if (session()->has('referral_code')) {
            $existingUser = User::where('email', $email)->first();
            if ($existingUser) {
                return response()->json([
                    'error' => true, 
                    'message' => 'This email already exists. Please use a different email address for your referral order.'
                ]);
            }
        }
    ...
    private function orderAddress($request, $state_name, $ship_state_name)
    {
        ...
        $address = [
         ...
         // Only create site user if the user came from a referral code (brand ambassador referral)
         if (session()->has('referral_code')) {
    ...
    public function afterPayCheckout(Request $request)
    {
        ...
        if (session()->has('referral_code')) {
            ...
            if ($existingUser) {
                return response()->json([
                    'error' => true, 
                    'data' => 'This email already exists. Please use a different email address for your referral order.'
                ]);
            }
        }
